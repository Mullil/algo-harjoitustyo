...
The FFNN, Layer and Tensor classes are all tested together in end-to-end tests in the ffnn_test.py file. The tests check that the weights are updated after each backward pass and that the network is able to gain 100 % training accuracy on a small sample (50 instances) from the training data after a reasonable number of epochs. 
